...
\begin{enumerate}
    ...
    The most challenging part of creating a dataset is when we start the sampling itself. We want to keep in mind the variety of samples, meaning angles, positions and additionaly when it comes to dynamic gestures, various speeds and starting positions. Also the factor of overlapping gestures must be taken into considiration. 

    Despite introducing sampling specifically for dynamic gestures, it can still be difficult to create a dataset, which will enforce the model to learn on characteristic movemnts. At the end, the trained model is just as good as the dataset it was trained on.
