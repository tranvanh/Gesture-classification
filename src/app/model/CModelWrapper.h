...
class CModelWrapper {
public:
	CModelWrapper(const std::string& model_dir, const int& t, const int& f) :model(model_dir), num_timestep(t), num_features(f) {};
	int preditct(const std::vector<std::vector<float>> input_data);

private:
	cppflow::model model;
	int num_timestep;
	int num_features;
	std::vector<float> max_scaler = { 178.82447815, 178.68522644, 178.87007141, 179.13853455,
	...
	std::vector<float> min_scaler = { 9.13711243e+01, 9.14782333e+01, 9.10166931e+01, 9.16859894e+01,
	...
	void min_max_scaler(std::vector<float>& output, const std::vector<std::vector<float>> input);
};
