...
typedef enum _eLeapRS {
  ...
  /**
   * The connection to the service was unexpectedly closed while reading or
   * writing a message. The server may have terminated.
   * @since 3.0.0
   */
   ...
   *
   * Provided that the token value which identifies the request itself was, at
   * one point, valid, this error condition occurs when the request to which the
   * token refers has already been satisfied or is currently being satisfied.
   * @since 3.0.0
   */
   ...
   *
   * Provided that frame ID was, at one point, valid, this error condition
   * occurs when the identifier refers to a frame that occurred further in the
   * past than is currently recorded in the rolling frame window.
   * @since 3.0.0
   */
   ...
   *
   * The related routine can only operate on time points having occurred in the
   * past, and the provided timestamp occurs in the future.
   * @since 3.1.2
   */
   ...
   *
   * The related routine can only operate on time points occurring within its
   * immediate record of the past.
   * @since 3.1.2
   */
  ...
  /**
   * The requested operation can only be performed while the device is sending
   * data.
   * @since 3.0.0
   */
  ...
  /**
   * The specified device could not be opened. It is possible that the device
   * identifier is invalid, or that the device has been disconnected since being
   * enumerated.
   * @since 3.0.0
   */
...
typedef enum _eLeapTrackingMode {
  ...
  /** Tracking mode is not known (allows triggering of a new
     LEAP_TRACKING_MODE_EVENT) @since 5.0.0 */
...
typedef struct _LEAP_CONNECTION_CONFIG {
  ...
  /**
   * Specifies the server namespace to be used. Leave NULL to use the default
   * namespace.
   *
   * It is possible to launch the service with a different IPC connection
   * namespace (using internal service functions). Clients wishing to connect to
   * a different server namespace may specify that namespace here.
   *
 ...
 * tracking frames.
 *
 * The returned counter value is given in microseconds since an epoch time. The
 * clock used for the counter itself is implementation-defined, but generally
 ...
 *
 * @param pConfig The configuration to be used with the newly created
 * connection. If pConfig is null, a connection is created with a default
 * configuration.
 * @param[out] phConnection Receives a pointer to the connection object, set to
 * invalid on failure
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapCreateConnection(
    const LEAP_CONNECTION_CONFIG *pConfig, LEAP_CONNECTION *phConnection);

/** \ingroup Functions
 * Opens a connection to the service.
 *
 * This routine will not block. A connection to the service will not be
 * established until the first invocation of LeapPollConnection.
 *
...
/**  \ingroup Structs
 * Received from LeapPollConnection() when a connection to the Ultraleap
 * Tracking Service is established.
 * @since 3.0.0
 */
...
/** \ingroup Structs
 * Received from LeapPollConnection() when a connection to the Ultraleap
 * Tracking Service is lost.
 *
 * If a LeapC function that performs a transaction with the Ultraleap Tracking
 * Service is called after the connection is lost, the next call to
 * LeapPollConnection() will return this event. Otherwise, it can take up to 5
 * seconds of polling the connection to receive this event.
 * @since 3.0.0
 */
...
typedef enum _eLeapConnectionStatus {
   ...
   * Call LeapOpenConnection() to open a connection to the Ultraleap Tracking
   * Service.
   * @since 3.0.0
   */
  ...
  /**
   * The connection could not be opened because the Ultraleap Tracking Service
 ...
 * LeapGetConnectionInfo(), which creates this struct, to check the connection
 * status.
 * @since 3.0.0
 */
 ...
 * LeapCreateConnection.
 * @param[out] pInfo A pointer to a buffer that receives additional connection
 * information. One input, the size field of pInfo is the size of the
 * buffer(i.e. the size of a LEAP_CONNECTION_INFO struct); On output, the size
 * field of pInfo receives the size necessary to hold the entire information
 * block.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL
LeapGetConnectionInfo(LEAP_CONNECTION hConnection, LEAP_CONNECTION_INFO *pInfo);
...
typedef enum _eLeapPolicyFlag {
  ...
  /** The policy specifying whether to automatically stream images from the
     device. @since 4.0.0 */
  eLeapPolicyFlag_Images = 0x00000002,

  /** The policy specifying whether to optimize tracking for head-mounted
     device. @since 3.0.0 */
 ...
 * call to LeapPollConnection provides a LEAP_POLICY_EVENT containing the
 * current policies, reflecting any changes.
 *
 * To get the current policies without changes, specify zero for both the set
 * and clear parameters. When ready, LeapPollConnection() provides a
 * LEAP_POLICY_EVENT containing the current settings.
 *
 ...
 * @param set A bitwise combination of flags to be set. Set to 0 if not setting
 * any flags.
 * @param clear A bitwise combination of flags to be cleared. Set to 0 to if not
 * clearing any flags.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapSetPolicyFlags(LEAP_CONNECTION hConnection,
 ...
 * @param set A bitwise combination of flags to be set. Set to 0 if not setting
 * any flags.
 * @param clear A bitwise combination of flags to be cleared. Set to 0 if not
 * clearing any flags.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 5.4.0
 */
/*
LEAP_EXPORT eLeapRS LEAP_CALL LeapSetPolicyFlagsEx(LEAP_CONNECTION hConnection,
LEAP_DEVICE hDevice, uint64_t set, uint64_t clear);
       */

/** \ingroup Functions
 * Requests a tracking mode.
 *
 * Changing tracking modes is asynchronous. After you call this function, a
 *subsequent call to LeapPollConnection provides a LEAP_POLICY_EVENT containing
 *the current policies, reflecting any changes.
 *
 ...
 * @since 5.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapSetTrackingMode(LEAP_CONNECTION hConnection,
                                                  eLeapTrackingMode mode);
 ...
 */
/*
LEAP_EXPORT eLeapRS LEAP_CALL LeapSetTrackingModeEx(LEAP_CONNECTION hConnection,
LEAP_DEVICE hDevice, eLeapTrackingMode mode);
       */

/** \ingroup Functions
 * Requests the currently set tracking mode.
 *
 * Requesting the current tracking mode is asynchronous. After you call this
 * function, a subsequent call to LeapPollConnection provides a
 * LEAP_TRACKING_MODE_EVENT containing the current tracking mode, reflecting any
 * changes.
 *
 * The eLeapTrackingMode enumeration defines the tracking mode.
 *
 ...
 */
/*
LEAP_EXPORT eLeapRS LEAP_CALL LeapGetTrackingModeEx(LEAP_CONNECTION hConnection,
LEAP_DEVICE hDevice);
       */
 ...
 * This is treated as a 'user pause', as though a user had requested a pause
 * through the Leap Control Panel. The connection must have the AllowPauseResume
 * policy set or it will fail with eLeapRS_InvalidArgument.
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 ...
 * @since 4.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapSetPause(LEAP_CONNECTION hConnection,
                                           bool pause);

/** \ingroup Functions
 * Sets the allocator functions to use for a particular connection.
 *
 * If user-supplied allocator functions are not supplied, the functions that
 * require dynamic memory allocation will not be available.
 *
 * @param hConnection A handle to the connection object, created by
 * LeapCreateConnection().
 * @param allocator A pointer to a structure containing the allocator functions
 * to be called as needed by the library.
 * @since 4.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapSetAllocator(LEAP_CONNECTION hConnection,
 ...
 * fetched asynchronously since it requires a service transaction.
 * LeapPollConnection() returns this event structure when the request has been
 * processed. Use the requestID value to correlate the response to the
 * originating request.
 * @since 3.0.0
 */
typedef struct _LEAP_CONFIG_RESPONSE_EVENT {
  ...
  /**
   * The configuration value retrieved from the service. Do not free any memory
   * pointed to by this member. The value held is only valid until the next call
   * to LeapPollConnection().
   * @since 3.0.0
   */
  LEAP_VARIANT value;
 ...
 * returns this event structure when the request has been processed. Use the
 * requestID value to correlate the response to the originating request.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.0.0
 */
...
/** \ingroup Functions
 * Causes the client to commit a configuration change to the Ultraleap Tracking
 * Service.
 *
 * The change is performed asynchronously -- and may fail. LeapPollConnection()
 * returns this event structure when the request has been processed. Use the
 * pRequestID value to correlate the response to the originating request.
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param key The key of the configuration to commit.
 * @param value The value of the configuration to commit.
 * @param[out] pRequestID A pointer to a memory location to which the id for
 * this request is written, or nullptr if this value is not needed.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapSaveConfigValue(LEAP_CONNECTION hConnection,
                                                  const char *key,
 ...
 * LeapPollConnection() returns this event structure when the request has been
 * processed. Use the pRequestID value to correlate the response to the
 * originating request.
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param key The key of the configuration to request
 * @param[out] pRequestID A pointer to a memory location to which the id for
 * this request is written.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapRequestConfigValue(
    LEAP_CONNECTION hConnection, const char *key, uint32_t *pRequestID);

/** \ingroup Functions
 * Retrieves a list of Ultraleap Tracking camera devices currently attached to
 * the system.
 *
 * To get the number of connected devices, call this function with the pArray
 * parameter set to null. The number of devices is written to the memory
 * specified by pnArray. Use the device count to create an array of
 * LEAP_DEVICE_REF structs large enough to hold the number of connected devices.
 * Finally, call LeapGetDeviceList() with this array and known count to get the
 * list of Leap devices. A device must be opened with LeapOpenDevice() before
 * device properties can be queried.
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param[out] pArray A pointer to an array that LeapC fills with the device
 * list.
 * @param[in,out] pnArray On input, set to the number of elements in pArray; on
 * output, LeapC sets this to the number of valid device handles.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapGetDeviceList(LEAP_CONNECTION hConnection,
 ...
 * @since 3.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapOpenDevice(LEAP_DEVICE_REF rDevice,
 ...
 * and if \p unsubscribeOthers is \c true, then unsubscribes from all other
 * devices as well (see LeapUnsubscribeEvents()).
 *
 ...
 * @since 5.4.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapSetPrimaryDevice(LEAP_CONNECTION hConnection,
                                                   LEAP_DEVICE hDevice,
                                                   bool unsubscribeOthers);
 ...
 * @since 5.4.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapSubscribeEvents(LEAP_CONNECTION hConnection,
                                                  LEAP_DEVICE hDevice);
 ...
 * @since 5.4.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapUnsubscribeEvents(LEAP_CONNECTION hConnection,
                                                    LEAP_DEVICE hDevice);
...
typedef struct _LEAP_DEVICE_INFO {
   ...
   *
   * The baseline is defined as the distance between the center axis of each
   * lens in a stereo camera system. For other camera systems, this value is set
   * to zero.
   * @since 3.0.0
   */
  uint32_t baseline;

  /** The required length of the serial number char buffer including the null
   * character. @since 3.0.0 */
 ...
 *
 * To get the device serial number, you must supply a LEAP_DEVICE_INFO struct
 * whose serial member points to a char array large enough to hold the
 * null-terminated serial number string. To get the required length, call
 * LeapGetDeviceInfo() using a LEAP_DEVICE_INFO struct that has serial set to
 * NULL. LeapC sets serial_length field of the struct to the required length.
 * You can then allocate memory for the string, set the serial field, and call
 * this function again.
 *
 ...
 * @since 3.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapGetDeviceInfo(LEAP_DEVICE hDevice,
 ...
 *
 * Note that the matrix is in column major, e.g. transform[12] corresponds to
 * the x coordinate of the translation vector t.
 *
 * A possible pipeline would be, for example:
 * 1) Get "palm_pos" the position of the center of the palm (as a 3x1 vector)
 * 2) Construct a 4x1 vector using the palm_position: palm_pos_4 = (palm_pos.x;
 * palm_pos.y; palm_pos.z; 1.0f) 3) Create a 4x4 matrix "trans_mat" as
 * illustrated above using the returned transform 4) Get the position of the
 * center of the palm in world coordinates by multiplying trans_mat and
 * palm_pos_4: center_world_4 = trans_mat * palm_pos_4
 *
 * This function returns eLeapRS_Unsupported in the case where this
 * functionality is not yet supported.
 *
 * @param hDevice A handle to the device to be queried.
 * @param[out] transform A pointer to a single-precision float array of size 16,
 * containing the coefficients of the 4x4 matrix in Column Major order.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 5.4.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapGetDeviceTransform(LEAP_DEVICE hDevice,
                                                     float *transform);
...
typedef struct _LEAP_DEVICE_EVENT {
  ...
  /** The status of the connected device. A combination of flags from the
   * eLeapDeviceStatus collection. */
  uint32_t status;
} LEAP_DEVICE_EVENT;
...
typedef enum _eLeapDeviceStatus {
  ...
  /**
   * There are known sources of infrared interference. Device has transitioned
   * to robust mode in order to compensate.  @since 3.1.3
   */
  ...
  /** The device reports corrupt firmware or cannot install a required firmware
     update. @since 3.0.0 */
...
typedef struct _LEAP_DEVICE_FAILURE_EVENT {
  ...
  /**
   * A handle to the device generating this failure event, if available,
   * otherwise NULL.
   *
...
typedef struct _LEAP_FRAME_HEADER {
   ...
   *
   * All frames carrying this frame ID are part of the same unit of processing.
   * This counter is generally an increasing counter, but may reset to another
   * value if the user stops and restarts streaming.
   *
   * For interpolated frames, this value corresponds to the identifier of the
   * frame upper bound.
   * @since 3.0.0
   */
  int64_t frame_id;
...
LEAP_STATIC_ASSERT(sizeof(eLeapCameraCalibrationType) == 4,
                   "Incorrect enum size");
 ...
 *
 * Each point in the grid contains the coordinates of the pixel in the image
 * buffer that contains the data for the pixel in the undistorted image
 * corresponding to that point in the grid. Interpolate between points in the
 * matrix to correct image pixels that don't fall directly underneath a point in
 * the distortion grid.
 *
...
typedef struct _LEAP_VECTOR {
  /** You can access the vector members as either an array or individual float
   * values. */
  union {
...
typedef struct _LEAP_DIGIT {
  ...
  union {
    ...
    struct {
       ...
       * For thumbs, this bone is set to have zero length and width, an identity
       * basis matrix, and its joint positions are equal. Note that this is
       * anatomically incorrect; in anatomical terms, the intermediate phalange
       * is absent in a real thumb, rather than the metacarpal bone. In the
       * Ultraleap Tracking model, however, we use a "zero" metacarpal bone
       * instead for ease of programming.
       * @since 3.0.0
       */
...
typedef struct _LEAP_PALM {
  /**
   * The center position of the palm in millimeters from the Ultraleap Tracking
   * camera device origin.
   * @since 3.0.0
   */
  ...
  /**
   * The unit direction vector pointing from the palm position toward the
   * fingers.
   * @since 3.0.0
   */
...
typedef struct _LEAP_HAND {
  ...
  /**
   * How confident we are with a given hand pose. Not currently used
   * (always 1.0).
   * @since 3.0.0
   */
  float confidence;
 ...
 * moment in time. The LEAP_FRAME struct is the container for all the tracking
 * data.
 * @since 3.0.0
 */
typedef struct _LEAP_TRACKING_EVENT {
  ...
  /**
   * An identifier for this tracking frame. This identifier is meant to be
   * monotonically increasing, but values may be skipped if the client
   * application does not poll for messages fast enough. This number also
   * generally increases at the same rate as info.frame_id, but if the server
   * cannot process every image received from the device cameras, the
   * info.frame_id identifier may increase faster.
   * @since 3.0.0
   */
   ...
   *
   * This frame rate is distinct from the image frame rate, which is the rate
   * that images are being read from the device. Depending on host CPU
   * limitations, the tracking frame rate may be substantially less than the
   * device frame rate.
   *
   * This number is generally equal to or less than the device frame rate, but
   * there is one case where this number may be _higher_ than the device frame
   * rate:  When the device rate drops. In this case, the device frame rate will
   * fall sooner than the tracking frame rate.
   *
   * This number is equal to zero if there are not enough frames to estimate
   * frame rate.
   *
...
typedef enum _eLeapLogSeverity {
  ...
  /** A message about a fault that could render the software or device
     non-functional. @since 3.0.0 */
  eLeapLogSeverity_Critical,
  /** A message warning about a condition that could degrade device
     capabilities. @since 3.0.0 */
...
/** \ingroup Structs
 * A notification that a device's status has changed. One of these messages is
 * received by the client as soon as the service is connected, or when a new
 * device is attached.
 * @since 3.1.3
 */
typedef struct _LEAP_DEVICE_STATUS_CHANGE_EVENT {
  ...
  /** The last known status of the device. This is a combination of
   * eLeapDeviceStatus flags. @since 3.1.3*/
  uint32_t last_status;

  /** The current status of the device. This is a combination of
   * eLeapDeviceStatus flags. @since 3.1.3*/
  uint32_t status;
...
typedef struct _LEAP_IMAGE {
   ...
   *
   * The distortion matrix only changes when the streaming device changes or
   * when the device orientation flips -- inverting the image and the distortion
   * grid. Since building a matrix to undistort an image can be a time-consuming
   * task, you can optimize the process by only rebuilding this matrix (or
   * whatever data type you use to correct image distortion) when the grid
   * actually changes.
   */
  ...
  /** Offset, in bytes, from the beginning of the data ptr to the actual
...
typedef struct _LEAP_HEAD_POSE_EVENT {
  ...
  int64_t timestamp;
  /**
   * The position and orientation of the user's head. Positional tracking must
   * be enabled.
   * @since 4.1.0
   */
  ...
  /**
   * The linear and angular velocity of the user's head. Positional tracking
   * must be enabled.
   * @since 4.1.0
   */
 ...
 * the buffer containing the image data -- which was allocated using the
 * allocator function passed to LeapC using the LeapSetAllocator.
 * @since 4.0.0
 */
...
typedef enum _eLeapEventType {
  ...
  /**
   * A device has failed.
   * Device failure could be caused by hardware failure, USB controller issues,
   * or other system instability. Note that unplugging a device generates an
   ...
   * This can be due to setting a policy with LeapSetPolicyFlags() or due to
   * changing or policy-related config settings, including images_mode. (A user
   * can also change these policies using the Ultraleap Tracking Control Panel.)
   * @since 3.0.0
   */
   ...
   *
   * This event is generally asserted when the device has been detached from the
   * system, when the connection to the service has been lost, or if the device
   * is closed while streaming. Generally, any event where the system can
   * conclude no further frames will be received will result in this message.
   * The DeviceEvent field will be filled with the id of the formerly attached
   * device.
   * @since 3.0.0
   */
   ...
   * This can be due to changing the hmd or screentop policy with
   * LeapSetPolicyFlags(). or setting the tracking mode using
   * LeapSetTrackingMode().
   * @since 5.0.0
   */
  ...
  /**
   * A head pose. The message contains the timestamped head position and
   * orientation.
   * @since 4.1.0
   */
 ...
 *
 * The specific types of events that may be received are not configurable in
 * this entrypoint. Configure the device or connection object directly to change
 * what events will be received.
 *
 * Pointers in the retrieved event message structure will be valid until the
 * associated connection or device is closed, or the next call to
 * LeapPollConnection().
 *
 * Calling this method concurrently will return eLeapRS_ConcurrentPoll.
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param timeout The maximum amount of time to wait, in milliseconds. If this
 * value is zero, the evt pointer references the next queued message, if there
 * is one, and returns immediately.
 * @param[out] evt A pointer to a structure that is filled with event
 * information. This structure will be valid as long as the LEAP_CONNECTION
 * object is valid.
 *
 * @returns The operation result code, a member of the eLeapRS enumeration. If
 * the operation times out, this method will return eLeapRS_Timeout. The evt
 * pointer will reference a message of type eLeapEventType_None.
 * @since 3.0.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapPollConnection(LEAP_CONNECTION hConnection,
                                                 uint32_t timeout,
                                                 LEAP_CONNECTION_MESSAGE *evt);

/** \ingroup Functions
 * Retrieves the number of bytes required to allocate an interpolated frame at
 * the specified time.
 *
 * Use this function to determine the size of the buffer to allocate when
 * calling LeapInterpolateFrame().
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param timestamp The timestamp of the frame whose size is to be queried.
 * @param[out] pncbEvent A pointer that receives the number of bytes required to
 * store the specified frame.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.1.1
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapGetFrameSize(LEAP_CONNECTION hConnection,
                                               int64_t timestamp,
                                               uint64_t *pncbEvent);

/** \ingroup Functions
 * Retrieves the number of bytes required to allocate an interpolated frame at
 * the specified time for a particular device.
 *
 * Use this function to determine the size of the buffer to allocate when
 * calling LeapInterpolateFrameEx().
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param hDevice A device handle returned by LeapOpenDevice().
 * @param timestamp The timestamp of the frame whose size is to be queried.
 * @param[out] pncbEvent A pointer that receives the number of bytes required to
 ...
 * @since 5.4.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapGetFrameSizeEx(LEAP_CONNECTION hConnection,
                                                 LEAP_DEVICE hDevice,
                                                 int64_t timestamp,
                                                 uint64_t *pncbEvent);

/** \ingroup Functions
 * Constructs a frame at the specified timestamp by interpolating between
 * measured frames.
 *
 * Caller is responsible for allocating a buffer large enough to hold the data
 * of the frame. Use LeapGetFrameSize() to calculate the minimum size of this
 * buffer.
 *
 ...
 * the Ultraleap Tracking Service. This process is required to achieve accurate,
 * smooth interpolation.
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param timestamp The timestamp at which to interpolate the frame data.
 * @param[out] pEvent A pointer to a flat buffer which is filled with an
 * interpolated frame.
 * @param ncbEvent The number of bytes pointed to by pEvent.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.1.1
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapInterpolateFrame(LEAP_CONNECTION hConnection,
                                                   int64_t timestamp,
 ...
 *
 * Caller is responsible for allocating a buffer large enough to hold the data
 * of the frame. Use LeapGetFrameSizeEx() to calculate the minimum size of this
 * buffer.
 *
 * Use LeapCreateClockRebaser(), LeapUpdateRebase(), and LeapRebaseClock() to
 * synchronize time measurements in the application with time measurements in
 ...
 * @since 5.4.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapInterpolateFrameEx(
    LEAP_CONNECTION hConnection, LEAP_DEVICE hDevice, int64_t timestamp,
    LEAP_TRACKING_EVENT *pEvent, uint64_t ncbEvent);

/** \ingroup Functions
 * Constructs a frame at the specified timestamp by interpolating between a
 * frame near the timestamp and a frame near the sourceTimestamp.
 *
 * Caller is responsible for allocating a buffer large enough to hold the data
 * of the frame. Use LeapGetFrameSize() to calculate the minimum size of this
 * buffer.
 ...
 * @param sourceTimestamp The timestamp of the beginning frame from which to
 ...
 * @since 3.1.1
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapInterpolateFrameFromTime(
    LEAP_CONNECTION hConnection, int64_t timestamp, int64_t sourceTimestamp,
    LEAP_TRACKING_EVENT *pEvent, uint64_t ncbEvent);
 ...
 * @since 5.4.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapInterpolateFrameFromTimeEx(
    LEAP_CONNECTION hConnection, LEAP_DEVICE hDevice, int64_t timestamp,
    int64_t sourceTimestamp, LEAP_TRACKING_EVENT *pEvent, uint64_t ncbEvent);
 ...
 *
 * This method closes the specified connection object if it is opened, destroys
 * the underlying object, and releases all resources associated with it.
 *
 * This method never fails.
 *
 * Be sure that no other functions are accessing the connection when this
 * function is called.
 *
 ...
 *
 * @param[out] phClockRebaser The pointer to the clock-rebaser object to be
 * initialized.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.1.2
 */
LEAP_EXPORT eLeapRS LEAP_CALL
LeapCreateClockRebaser(LEAP_CLOCK_REBASER *phClockRebaser);

/** \ingroup Functions
 * Updates the relationship between the Ultraleap Tracking Service clock and the
 * user clock.
 *
 * When using LeapInterpolateFrame(), call this function for every graphics
 * frame rendered by your application. The function should be called as close to
 * the actual point of rendering as possible.
 *
 * The relationship between the application clock and the Ultraleap Tracking
 * Service clock is neither fixed nor stable. Simulation restarts can cause user
 * clock values to change instantaneously. Certain systems simulate slow motion,
 * or respond to heavy load, by reducing the tick rate of the user clock. As a
 * result, the LeapUpdateRebase() function must be called for every rendered
 * frame.
 *
 * @param hClockRebaser The handle to a rebaser object created by
 * LeapCreateClockRebaser().
 * @param userClock A clock value supplied by the application, sampled at about
 * the same time as LeapGetNow() was sampled.
 * @param leapClock The Ultraleap Tracking Service clock value sampled by a call
 * to LeapGetNow().
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.1.2
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapUpdateRebase(LEAP_CLOCK_REBASER hClockRebaser,
                                               int64_t userClock,
                                               int64_t leapClock);
 ...
 *
 * Use this function to translate your application clock to the Ultraleap
 * Tracking Service clock when interpolating frames. LeapUpdateRebase() must be
 * called for every rendered frame for the relationship between the two clocks
 * to remain synchronized.
 *
 * @param hClockRebaser The handle to a rebaser object created by
 * LeapCreateClockRebaser().
 * @param userClock The clock in microseconds referenced to the application
 * clock.
 * @param[out] pLeapClock The corresponding Ultraleap Tracking Service clock
 * value.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.1.2
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapRebaseClock(LEAP_CLOCK_REBASER hClockRebaser,
                                              int64_t userClock,
                                              int64_t *pLeapClock);
 ...
 *
 * This method destroys the specified clock-rebaser object, and releases all
 * resources associated with it.
 *
 * @param hClockRebaser A handle to the clock-rebaser object to be destroyed.
 * @since 3.1.2
 */
LEAP_EXPORT void LEAP_CALL
LeapDestroyClockRebaser(LEAP_CLOCK_REBASER hClockRebaser);

/** \ingroup Functions
 * Provides the corrected camera ray intercepting the specified point on the
 * image.
 *
 * Given a point on the image, ``LeapPixelToRectilinear()`` corrects for camera
 * distortion and returns the true direction from the camera to the source of
 * that image point within the Ultraleap Tracking camera field of view.
 *
 * This direction vector has an x and y component [x, y, 1], with the third
 * element always 1. Note that this vector uses the 2D camera coordinate system
 ...
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param camera The camera to use, a member of the eLeapPerspectiveType
 * enumeration
 * @param pixel A Vector containing the position of a pixel in the image.
 * @returns A Vector containing the ray direction (the z-component of the vector
 * is always 1).
 * @since 3.1.3
 */
LEAP_EXPORT LEAP_VECTOR LEAP_CALL
LeapPixelToRectilinear(LEAP_CONNECTION hConnection, eLeapPerspectiveType camera,
                       LEAP_VECTOR pixel);
 ...
 *
 * Given a point on the image, ``LeapPixelToRectilinearEx()`` corrects for
 * camera distortion and returns the true direction from the camera to the
 * source of that image point within the Devices field of view.
 *
 * This direction vector has an x and y component [x, y, 1], with the third
 * element always 1. Note that this vector uses the 2D camera coordinate system
 * where the x-axis parallels the longer (typically horizontal) dimension and
 ...
 */
LEAP_EXPORT LEAP_VECTOR LEAP_CALL
LeapPixelToRectilinearEx(LEAP_CONNECTION hConnection, LEAP_DEVICE hDevice,
 ...
 * ``LeapRectilinearToPixel()`` corrects for camera distortion and returns the
 ...
 *
 * The ``LeapRectilinearToPixel()`` function returns pixel coordinates outside
 * of the image bounds if you project a ray toward a point for which there is no
 * recorded data.
 *
 * ``LeapRectilinearToPixel()`` is typically not fast enough for realtime
 * distortion correction. For better performance, use a shader program executed
 * on a GPU.
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param camera The camera to use, a member of the eLeapPerspectiveType
 ...
 * @returns A Vector containing the pixel coordinates [x, y, 1] (with z always
 * 1).
 * @since 3.1.3
 */
LEAP_EXPORT LEAP_VECTOR LEAP_CALL
LeapRectilinearToPixel(LEAP_CONNECTION hConnection, eLeapPerspectiveType camera,
 ...
 *
 * Given a ray projected from the camera in the specified direction,
 * ``LeapRectilinearToPixelEx()`` corrects for camera distortion and returns the
 * corresponding pixel coordinates in the image.
 *
 * The ray direction is specified in relationship to the camera. The first
 * vector element is the tangent of the "horizontal" view angle; the second
 * element is the tangent of the "vertical" view angle.
 *
 * The ``LeapRectilinearToPixelEx()`` function returns pixel coordinates outside
 * of the image bounds if you project a ray toward a point for which there is no
 * recorded data.
 *
 * ``LeapRectilinearToPixelEx()`` is typically not fast enough for realtime
 * distortion correction. For better performance, use a shader program executed
 * on a GPU.
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 ...
 */
LEAP_EXPORT LEAP_VECTOR LEAP_CALL
LeapRectilinearToPixelEx(LEAP_CONNECTION hConnection, LEAP_DEVICE hDevice,
 ...
 * @since 3.2.1
 */
LEAP_EXPORT void LEAP_CALL LeapCameraMatrix(LEAP_CONNECTION hConnection,
 ...
 * @since 5.4.0
 */
LEAP_EXPORT void LEAP_CALL LeapCameraMatrixEx(LEAP_CONNECTION hConnection,
                                              LEAP_DEVICE hDevice,
                                              eLeapPerspectiveType camera,
                                              float *dest);
 ...
 * @param camera The camera to use, a member of the eLeapPerspectiveType
 * enumeration
 * @param[out] dest  A pointer to a single-precision float array of size 16,
 * containing the coefficients of the 4x4 matrix in Column Major order
 * @since 5.1.0
 */
LEAP_EXPORT void LEAP_CALL LeapExtrinsicCameraMatrix(
    LEAP_CONNECTION hConnection, eLeapPerspectiveType camera, float *dest);

/** \ingroup Functions
 *
 * Returns a transformation matrix from 3D Leap coordinate space to the
 * coordinate system of the requested camera This is composed of a 4 x 4 matrix
 * of the form:
 *
 * R, t <br>
 * 0, 1
 ...
 */
LEAP_EXPORT void LEAP_CALL
LeapExtrinsicCameraMatrixEx(LEAP_CONNECTION hConnection, LEAP_DEVICE hDevice,
                            eLeapPerspectiveType camera, float *dest);
 ...
 * @since 3.2.1
 */
LEAP_EXPORT void LEAP_CALL LeapDistortionCoeffs(LEAP_CONNECTION hConnection,
                                                eLeapPerspectiveType camera,
                                                float *dest);
 ...
 * @since 5.4.0
 */
LEAP_EXPORT void LEAP_CALL LeapDistortionCoeffsEx(LEAP_CONNECTION hConnection,
                                                  LEAP_DEVICE hDevice,
                                                  eLeapPerspectiveType camera,
                                                  float *dest);
 ...
 *
 * This method is guaranteed to never return null for the LEAP_DEVICE_INFO.pid
 * field returned by a successful call to LeapGetDeviceInfo
 *
 * @param pid The pid of the device
 * @returns The string name of the device model, or null if the device type
 * string is invalid.
 *
 */
 ...
 * function. Also used in members of LEAP_RECORDING_PARAMETERS and
...
typedef struct _LEAP_RECORDING_STATUS {
  /** Some combination of eLeapRecordingFlags indicating the status of the
   * recording. @since 3.2.0 */
  uint32_t mode;
} LEAP_RECORDING_STATUS;
 ...
 *
 * Pass the LEAP_RECORDING pointer to LeapRecordingOpen() to initiate reading
 * from or writing to a recording. The recording path is relative to the "user
 * path" which is the SD card on Android.
 *
 ...
 * @param params The LEAP_RECORDING_PARAMETERS describing what operations are
 * requested.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.2.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL
LeapRecordingOpen(LEAP_RECORDING *ppRecording, const char *filePath,
                  LEAP_RECORDING_PARAMETERS params);

/** \ingroup Functions
 * Closes a LEAP_RECORDING.
 *
 * @param[out] ppRecording The recording being closed. Will modify *ppRecording
 * to be null.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.2.0
 */
 ...
 * @param[out] pstatus A LEAP_RECORDING_STATUS struct to receive the recording
 * status.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.2.0
 */

LEAP_EXPORT eLeapRS LEAP_CALL LeapRecordingGetStatus(
    LEAP_RECORDING pRecording, LEAP_RECORDING_STATUS *pstatus);

/** \ingroup Functions
 * Retrieves the number of bytes required to allocate the next frame in a
 * recording.
 *
 * Use this function to determine the size of the buffer to allocate before
 * calling LeapRecordingRead().
 *
 ...
 * @since 3.2.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapRecordingReadSize(LEAP_RECORDING pRecording,
                                                    uint64_t *pncbEvent);
 ...
 *
 * @param pRecording The recording being read from.
 * @param[out] pEvent A pointer to a flat buffer which is filled with the next
 * recorded frame.
 * @param ncbEvent The number of bytes pointed to by pEvent.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 3.2.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapRecordingRead(LEAP_RECORDING pRecording,
                                                LEAP_TRACKING_EVENT *pEvent,
                                                uint64_t ncbEvent);
 ...
 * @since 3.2.0
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapRecordingWrite(LEAP_RECORDING pRecording,
                                                 LEAP_TRACKING_EVENT *pEvent,
 ...
 * @since 5.x.x
 */
LEAP_EXPORT void LEAP_CALL LeapScaleOffsetMatrix(LEAP_CONNECTION hConnection,
                                                 eLeapPerspectiveType camera,
                                                 float *dest);
 ...
 * to project 3D points into a rectilinear image space (i.e. to visualise hands
 * on an undistorted image).
 *
 * The pipeline would be:
 * 1) Take 3D points from hand tracking.
 * 2) Apply an extrinsic transformation to a specific camera's coordinate system
 * (@sa LeapExtrinsicCameraMatrixEx) 3) Apply a perspective division to
 * transform 3D points to rays. 4) Apply the ScaleOffset matrix to these points.
 *
 * These points will now be in the correct coordinate system consistent with the
 * undistorted rectilinear image provided by LEAP_IMAGE::distortion_matrix.
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param hDevice A device handle returned by LeapOpenDevice().
 * @param camera The camera to use, a member of the eLeapPerspectiveType
 ...
 * @since 5.x.x
 */
LEAP_EXPORT void LEAP_CALL LeapScaleOffsetMatrixEx(LEAP_CONNECTION hConnection,
                                                   LEAP_DEVICE hDevice,
                                                   eLeapPerspectiveType camera,
                                                   float *dest);
 ...
 *
 * If an invalid connection handle is provided only the version details of the
 * client will be available.
 *
 * @param hConnection The connection handle created by LeapCreateConnection().
 * @param versionPart The version part to return, this will reference one part
 * of the system.
 * @param[out] pVersion A pointer to a struct used to store the version number.
 * @returns The operation result code, a member of the eLeapRS enumeration.
 * @since 5.2.x
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapGetVersion(LEAP_CONNECTION hConnection,
                                             eLeapVersionPart versionPart,
                                             LEAP_VERSION *pVersion);

/**
 * Not Supported:
 *
 * The following functions are included for API compatibility with earlier LeapC
 * version. They are no longer supported and calling them will have no effects.
 */

LEAP_EXPORT eLeapRS LEAP_CALL
LeapGetPointMappingSize(LEAP_CONNECTION hConnection, uint64_t *pSize);
LEAP_EXPORT eLeapRS LEAP_CALL
LeapGetPointMapping(LEAP_CONNECTION hConnection,
                    LEAP_POINT_MAPPING *pointMapping, uint64_t *pSize);
...
LEAP_EXPORT eLeapRS LEAP_CALL LeapTelemetryProfiling(
    LEAP_CONNECTION hConnection, const LEAP_TELEMETRY_DATA *telemetryData);
LEAP_EXPORT uint64_t LEAP_CALL LeapTelemetryGetNow();

LEAP_EXPORT eLeapRS LEAP_CALL
LeapInterpolateHeadPose(LEAP_CONNECTION hConnection, int64_t timestamp,
                        LEAP_HEAD_POSE_EVENT *pEvent);
LEAP_EXPORT eLeapRS LEAP_CALL
LeapInterpolateHeadPoseEx(LEAP_CONNECTION hConnection, LEAP_DEVICE hDevice,
                          int64_t timestamp, LEAP_HEAD_POSE_EVENT *pEvent);
LEAP_EXPORT eLeapRS LEAP_CALL LeapInterpolateEyePositions(
    LEAP_CONNECTION hConnection, int64_t timestamp, LEAP_EYE_EVENT *pEvent);
